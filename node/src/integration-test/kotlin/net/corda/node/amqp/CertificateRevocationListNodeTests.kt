...
class CertificateRevocationListNodeTests {
    ...
    fun `Simple AMPQ Client to Server connection works`() {
        val (amqpServer, _) = createServer(serverPort)
        amqpServer.use {
            amqpServer.start()
    ...
    fun `AMPQ Client to Server connection fails when client's certificate is revoked`() {
        ...
        amqpServer.use {
            ...
            val (amqpClient, clientCert) = createClient()
    ...
    fun `AMPQ Client to Server connection fails when servers's certificate is revoked`() {
        val (amqpServer, serverCert) = createServer(serverPort)
    ...
    private fun createClient(): Pair<AMQPClient, X509Certificate> {
        ...
        val nodeCert = clientConfig.recreateNodeCaAndTlsCertificates(
                "http://${server.hostAndPort}/crl/node.crl",
                "http://${server.hostAndPort}/crl/empty.crl")
        ...
        return Pair(AMQPClient(
                ...
                clientTruststore, true), nodeCert)
    }

    private fun createServer(port: Int, name: CordaX500Name = ALICE_NAME): Pair<AMQPServer, X509Certificate> {
        ...
        val nodeCert = serverConfig.recreateNodeCaAndTlsCertificates(
                "http://${server.hostAndPort}/crl/node.crl",
                "http://${server.hostAndPort}/crl/empty.crl")
    ...
    private fun SSLConfiguration.recreateNodeCaAndTlsCertificates(nodeCaCrlDistPoint: String, tlsCrlDistPoint: String): X509Certificate {
        ...
        val newTlsCert = replaceCrlDistPointCaCertificate(tlsCert, CertificateType.TLS, nodeKeys, tlsCrlDistPoint, CordaX500Name.build(ROOT_CA.certificate.subjectX500Principal).x500Name)
